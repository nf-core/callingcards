...
workflow PREPARE_GENOME {
    ...
    if(params.regions_mask){
        ...
        ch_fasta = BEDTOOLS_MASKFASTA.out.fasta.map{ meta, fasta -> fasta}
    ...
    if(params.aligner == 'bwamem2'){
        ...
        if (!params.bwamem2_index){
            BWAMEM2_INDEX ( ch_fasta.map{it -> ["", it]}  )
        ...
        if (!params.bowtie2_index){
            BOWTIE2_BUILD ( ch_fasta.map{it -> ["", it]}  )
